...
fun GoogleMap.loadMapStyle(context: Context, resourceId: Int) {
    try {
        val success = setMapStyle(com.google.android.gms.maps.model.MapStyleOptions.loadRawResourceStyle(context, resourceId))
        if (!success) {
            android.util.Log.e("ERROR", "Style parsing failed.")
        }
    } catch (e: Resources.NotFoundException) {
        android.util.Log.e("ERROR", "Can't find style. Error: ", e)
    }
}

fun GoogleMap.initUiSettings() {
    kotlin.with(uiSettings) {
